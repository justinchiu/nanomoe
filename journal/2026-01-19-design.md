...
### Step 4: Refactor `DataBuffer` into a batch mixer
- `SampleSource` protocol yields `Sample`
- `DataBuffer` mixes sources (weighted/round-robin), filters zero-weight samples
- Packs via `pack_sequences()` until `max_tokens_per_batch` reached
- Optional prefetch queue for double-buffering

### Step 5: Add `unified_loss()` (`train/loss.py`)
- New file with single loss function
- Handles all tasks via `token_weights`

### Step 6: Simplify `grpo.py`
- Remove inline loss computation
- Use `unified_loss()`
- Keep advantage computation in `compute_grpo_advantages()`

### Step 7: Update `src/nanomoe/experiments/pretrain.py`
...
| `src/nanomoe/experiments/pretrain.py` | Use unified batch + loss |
